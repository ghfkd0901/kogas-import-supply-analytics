"""
# 📌 데이터 출처 및 사용 방법

## 1. 한국가스공사_한국의 대륙별 천연가스 수입현황

- 원본 데이터: 공공데이터포털(data.go.kr)
- 데이터셋 이름: **한국가스공사_한국의 대륙별 천연가스 수입현황**
- 링크: https://www.data.go.kr/data/15088508/fileData.do

### 이 코드에서의 활용 방식

- 원본 CSV 파일 경로  
  `D:\Project\한국가스공사월별시도별도시가스판매현황\data\한국가스공사_한국의 대륙별 천연가스 수입 현황_20240630.csv`
- 주요 원본 컬럼 예시
  - `연월일`
  - `단위가격(톤당달러)_아시아`, `금액(백만달러)_아시아`, `중량(톤)_아시아`
  - `단위가격(톤당달러)_북아메리카`, `금액(백만달러)_북아메리카`, `중량(톤)_북아메리카`
  - `...`, `기타` 등 대륙별 단위가격/금액/중량 컬럼

### 전처리/가공 로직 개요

1. 인코딩을 `utf-8` → 실패 시 `cp949` 순서로 시도하여 CSV 읽기
2. `중량(톤)기타` 컬럼명이 있을 경우 `중량(톤)_기타`로 정정
3. `_합계`로 끝나는 **합계 열은 모두 삭제**
   - 예: `단위가격(톤당달러)_합계`, `금액(백만달러)_합계`, `중량(톤)_합계`
4. 세 종류의 지표를 각각 long 포맷으로 변환(melt)
   - 단위가격: `연월일, 수입지역, 단위가격(톤당달러)`
   - 금액: `연월일, 수입지역, 금액(백만달러)`
   - 중량: `연월일, 수입지역, 중량(톤)`
5. 위 세 DataFrame을 `연월일 + 수입지역` 기준으로 병합하여 최종 long 포맷 생성
   - 최종 컬럼: `연월일, 수입지역, 단위가격(톤당달러), 금액(백만달러), 중량(톤)`
6. `연월일`을 `datetime`으로 변환하여 시계열 그래프에 사용

### 시각화(이 페이지에서 보여주는 내용)

1. **1번 그래프 – 단위금액(단위가격) 꺾은선 그래프**
   - y: `단위가격(톤당달러)`  
   - x: 집계 단위(연도별/월별)로 변환된 날짜  
   - 색: `수입지역`  
   - 단위가격이 `0`인 데이터는 그래프에서 제외

2. **2번 그래프 – 총금액**
   - 단위: 사이드바에서 `백만달러 / 억달러` 선택
   - 집계 단위: 사이드바에서 `연도별 / 월별` 선택
   - 타입:
     - 누적 영역그래프: 대륙별 금액 규모 추이
     - 100% 누적막대그래프: 시점별 **대륙 간 금액 비중 구조** 확인

3. **3번 그래프 – 중량**
   - 단위: 사이드바에서 `톤 / 천톤` 선택
   - 집계 단위: 사이드바에서 `연도별 / 월별` 선택
   - 타입:
     - 누적 영역그래프: 대륙별 수입 중량 규모 추이
     - 100% 누적막대그래프: 시점별 **대륙 간 중량 비중 구조** 확인

4. **하단 테이블**
   - 컬럼 순서: `연월일, 수입지역, 단위가격(톤당달러), 금액(백만달러), 중량(톤)`
   - 표시 형식:
     - `연월일` → `YYYY-MM`
     - `단위가격(톤당달러)` → 소수점 1자리
     - `금액(백만달러)`, `중량(톤)` → 반올림 정수 + 천단위 콤마

---

## 2. 한국가스공사_월별 시도별 도시가스 판매현황 (다른 페이지에서 사용)

- 원본 데이터: 공공데이터포털(data.go.kr)
- 데이터셋 이름: **한국가스공사_월별 시도별 도시가스 판매현황**
- 링크: https://www.data.go.kr/data/15040819/fileData.do

### (예시) 활용 방식 개요

이 데이터셋은 별도의 Streamlit 페이지에서 다음과 같이 활용할 수 있습니다.

- 주요 원본 컬럼 예시
  - `연월`
  - `강원`, `서울`, `경기`, `인천`, `경남`, `경북`, `광주`, `대구`, `대전`,
    `부산`, `세종`, `울산`, `전남`, `전북`, `제주`, `충남`, `충북` 등 시도별 판매량
- 전처리 예시
  1. `연월`을 날짜 또는 문자열(YYYY-MM)로 변환
  2. `pd.melt`를 사용하여  
     `연월, 시도, 판매량` 구조의 long 포맷으로 변환
  3. 필요 시 단위를 변환하거나(예: 천 단위, 백만 단위), 시도/연도별 집계 생성
- 시각화 예시
  - 시도별 월간 판매량 추세 꺾은선 그래프
  - 특정 연도 기준 시도별 막대그래프
  - 누적/비중 그래프 등

이 페이지(`대륙별_천연가스_수입현황.py`)에서는 **대륙별 수입 데이터만 사용**하며,  
시도별 판매현황 데이터는 다른 분석/시각화 페이지에서 활용할 수 있습니다.
"""

from pathlib import Path

import pandas as pd
import plotly.express as px
import streamlit as st

# ======================
# 기본 설정
# ======================
st.set_page_config(page_title="대륙별 천연가스 수입 현황", layout="wide")
st.title("🌍 한국의 대륙별 천연가스 수입 현황 분석")

# ---- 상대경로로 수정 ----
REPO_ROOT = Path(__file__).resolve().parents[1]
DATA_PATH = REPO_ROOT / "data" / "한국가스공사_한국의 대륙별 천연가스 수입 현황_20240630.csv"
FILE_PATH = DATA_PATH

# ======================
# 데이터 로드 & 정규화
# ======================
@st.cache_data
def load_and_transform(path: Path) -> pd.DataFrame:
    # CSV 읽기 (인코딩 자동)
    try:
        df = pd.read_csv(path, encoding="utf-8")
    except UnicodeDecodeError:
        df = pd.read_csv(path, encoding="cp949")

    # 1) 잘못된 컬럼명 보정: "중량(톤)기타" -> "중량(톤)_기타"
    if "중량(톤)기타" in df.columns:
        df = df.rename(columns={"중량(톤)기타": "중량(톤)_기타"})

    # 2) 합계 열 완전히 제거
    df = df.drop(columns=[c for c in df.columns if c.endswith("_합계")])

    # 3) 지표별 컬럼 리스트
    price_cols = [c for c in df.columns if "단위가격(톤당달러)" in c]
    money_cols = [c for c in df.columns if "금액(백만달러)" in c]
    weight_cols = [c for c in df.columns if "중량(톤)" in c]

    # 4) melt: 단위가격
    df_price = df.melt(
        id_vars=["연월일"],
        value_vars=price_cols,
        var_name="tmp",
        value_name="단위가격(톤당달러)",
    )
    df_price["수입지역"] = df_price["tmp"].str.replace(
        r"단위가격\(톤당달러\)_", "", regex=True
    )
    df_price = df_price.drop(columns=["tmp"])

    # 5) melt: 금액
    df_money = df.melt(
        id_vars=["연월일"],
        value_vars=money_cols,
        var_name="tmp",
        value_name="금액(백만달러)",
    )
    df_money["수입지역"] = df_money["tmp"].str.replace(
        r"금액\(백만달러\)_", "", regex=True
    )
    df_money = df_money.drop(columns=["tmp"])

    # 6) melt: 중량
    df_weight = df.melt(
        id_vars=["연월일"],
        value_vars=weight_cols,
        var_name="tmp",
        value_name="중량(톤)",
    )
    df_weight["수입지역"] = df_weight["tmp"].str.replace(
        r"중량\(톤\)_", "", regex=True
    )
    df_weight = df_weight.drop(columns=["tmp"])

    # 7) 병합 → 최종 long 데이터
    df_long = (
        df_price
        .merge(df_money, on=["연월일", "수입지역"])
        .merge(df_weight, on=["연월일", "수입지역"])
    )

    # 8) 날짜 형식 변환 (Timestamp)
    df_long["연월일"] = pd.to_datetime(df_long["연월일"])

    # 정렬
    df_long = df_long.sort_values(["연월일", "수입지역"]).reset_index(drop=True)

    return df_long


df = load_and_transform(FILE_PATH)

# ======================
# 사이드바 필터
# ======================
st.sidebar.header("⚙️ 필터")

# 집계 단위: 연도별 / 월별 (기본값: 연도별)
agg_level = st.sidebar.radio(
    "집계 단위",
    ["연도별", "월별"],
    index=0,        # 기본 연도별
    horizontal=True,
)

# 수입지역 선택
regions = sorted(df["수입지역"].unique())
selected_regions = st.sidebar.multiselect(
    "수입지역 선택",
    options=regions,
    default=regions,
)

# 날짜 범위 슬라이더(원본 일자 기준)
date_min_ts = df["연월일"].min()
date_max_ts = df["연월일"].max()

date_min = date_min_ts.date()
date_max = date_max_ts.date()

date_range = st.sidebar.slider(
    "기간 선택 (원자료 기준)",
    min_value=date_min,
    max_value=date_max,
    value=(date_min, date_max),
    format="YYYY-MM-DD",
)
start_date, end_date = date_range

# 총금액 단위 선택
amount_unit = st.sidebar.radio(
    "총금액 단위",
    ["백만달러", "억달러"],
    horizontal=True,
)

# 중량 단위 선택
weight_unit = st.sidebar.radio(
    "중량 단위",
    ["톤", "천톤"],
    horizontal=True,
)

# 2·3번 그래프 타입 선택 (공통)
chart_type = st.sidebar.radio(
    "2·3번 그래프 타입",
    ["누적 영역그래프", "100% 누적막대그래프(비율)"],
)

# 집계 단위에 따른 x축 라벨
x_label = "연도" if agg_level == "연도별" else "연월"

# ======================
# 필터 적용
# ======================
mask = (
    df["수입지역"].isin(selected_regions)
    & (df["연월일"].dt.date >= start_date)
    & (df["연월일"].dt.date <= end_date)
)
df_filtered = df[mask].copy()

# 집계용 날짜 컬럼 생성: 집계일
if agg_level == "연도별":
    # 연도 시작일(1월 1일)로 통일
    df_filtered["집계일"] = df_filtered["연월일"].dt.to_period("Y").dt.to_timestamp()
else:
    # 월 시작일(1일)로 통일
    df_filtered["집계일"] = df_filtered["연월일"].dt.to_period("M").dt.to_timestamp()

# ======================
# 단위 변환 함수들
# ======================
def convert_amount(df_in: pd.DataFrame, unit: str):
    df_out = df_in.copy()
    if unit == "백만달러":
        factor = 1
        y_label = "금액 (백만달러)"
    elif unit == "억달러":
        factor = 1 / 100  # 1억달러 = 100백만달러
        y_label = "금액 (억달러)"
    else:
        factor = 1
        y_label = "금액 (백만달러)"

    df_out["금액_변환"] = df_out["금액(백만달러)"] * factor
    return df_out, y_label


def convert_weight(df_in: pd.DataFrame, unit: str):
    df_out = df_in.copy()
    if unit == "톤":
        factor = 1
        y_label = "중량 (톤)"
    elif unit == "천톤":
        factor = 1 / 1000
        y_label = "중량 (천톤)"
    else:
        factor = 1
        y_label = "중량 (톤)"

    df_out["중량_변환"] = df_out["중량(톤)"] * factor
    return df_out, y_label


df_amount, amount_y_label = convert_amount(df_filtered, amount_unit)
df_weight, weight_y_label = convert_weight(df_filtered, weight_unit)

# ======================
# 1번 그래프: 단위금액 꺾은선 그래프 (단위가격 0 제외, 연/월 집계)
# ======================
st.subheader("1️⃣ 단위금액 추이 — 단위가격(톤당달러) 꺾은선 그래프")

df_line = df_filtered[df_filtered["단위가격(톤당달러)"] != 0].copy()

if df_line.empty:
    st.warning("단위가격이 0이 아닌 데이터가 없습니다. 필터를 다시 선택해 주세요.")
else:
    # 집계 단위별 평균 단위가격
    df_line_grouped = (
        df_line
        .groupby(["집계일", "수입지역"], as_index=False)["단위가격(톤당달러)"]
        .mean()
        .sort_values("집계일")
    )

    fig1 = px.line(
        df_line_grouped,
        x="집계일",
        y="단위가격(톤당달러)",
        color="수입지역",
        markers=True,
        title=f"대륙별 단위가격(톤당달러) 추이 ({agg_level} 평균, 단위가격 0 제외)",
    )
    fig1.update_layout(
        xaxis_title=x_label,
        yaxis_title="단위가격 (톤당달러)",
        hovermode="x unified",
    )
    st.plotly_chart(fig1, use_container_width=True)

# ======================
# 2번 그래프: 총금액 누적 영역 / 100% 누적막대 (연/월 집계)
# ======================
st.subheader("2️⃣ 총금액 — 누적 영역그래프 / 100% 누적막대그래프")

if df_amount.empty:
    st.warning("선택된 조건에 해당하는 데이터가 없습니다.")
else:
    # 집계 단위별 합계 금액
    df_amt_grouped = (
        df_amount
        .groupby(["집계일", "수입지역"], as_index=False)["금액_변환"]
        .sum()
        .sort_values("집계일")
    )

    if chart_type == "누적 영역그래프":
        fig2 = px.area(
            df_amt_grouped,
            x="집계일",
            y="금액_변환",
            color="수입지역",
            title=f"대륙별 {amount_y_label} 추이 ({agg_level}, 누적 영역)",
        )
        fig2.update_layout(
            xaxis_title=x_label,
            yaxis_title=amount_y_label,
            hovermode="x unified",
        )
        st.plotly_chart(fig2, use_container_width=True)

    else:
        # 100% 누적막대 (비중)
        df_pct_amt = df_amt_grouped.copy()
        total_by_date_amt = df_pct_amt.groupby("집계일")["금액_변환"].transform("sum")
        df_pct_amt["비중(%)"] = df_pct_amt["금액_변환"] / total_by_date_amt * 100

        fig2 = px.bar(
            df_pct_amt,
            x="집계일",
            y="비중(%)",
            color="수입지역",
            title=f"대륙별 금액 비중 (100% 누적막대, {agg_level}, 기준: {amount_unit})",
        )
        fig2.update_layout(
            xaxis_title=x_label,
            yaxis_title="비중 (%)",
            barmode="stack",
            hovermode="x unified",
        )
        st.plotly_chart(fig2, use_container_width=True)

# ======================
# 3번 그래프: 중량 누적 영역 / 100% 누적막대 (연/월 집계)
# ======================
st.subheader("3️⃣ 중량 — 누적 영역그래프 / 100% 누적막대그래프")

if df_weight.empty:
    st.warning("선택된 조건에 해당하는 데이터가 없습니다.")
else:
    # 집계 단위별 합계 중량
    df_wgt_grouped = (
        df_weight
        .groupby(["집계일", "수입지역"], as_index=False)["중량_변환"]
        .sum()
        .sort_values("집계일")
    )

    if chart_type == "누적 영역그래프":
        fig3 = px.area(
            df_wgt_grouped,
            x="집계일",
            y="중량_변환",
            color="수입지역",
            title=f"대륙별 {weight_y_label} 추이 ({agg_level}, 누적 영역)",
        )
        fig3.update_layout(
            xaxis_title=x_label,
            yaxis_title=weight_y_label,
            hovermode="x unified",
        )
        st.plotly_chart(fig3, use_container_width=True)

    else:
        # 100% 누적막대 (비중)
        df_pct_wgt = df_wgt_grouped.copy()
        total_by_date_wgt = df_pct_wgt.groupby("집계일")["중량_변환"].transform("sum")
        df_pct_wgt["비중(%)"] = df_pct_wgt["중량_변환"] / total_by_date_wgt * 100

        fig3 = px.bar(
            df_pct_wgt,
            x="집계일",
            y="비중(%)",
            color="수입지역",
            title=f"대륙별 중량 비중 (100% 누적막대, {agg_level}, 기준: {weight_unit})",
        )
        fig3.update_layout(
            xaxis_title=x_label,
            yaxis_title="비중 (%)",
            barmode="stack",
            hovermode="x unified",
        )
        st.plotly_chart(fig3, use_container_width=True)

# ======================
# 하단: 사람이 보기 좋게 포맷팅된 데이터 테이블 (원본 일자 기준)
# ======================
st.divider()
st.subheader("🔎 필터 적용된 데이터 (포맷팅, 원자료 기준)")

if df_filtered.empty:
    st.info("표시할 데이터가 없습니다.")
else:
    df_table = df_filtered.copy()

    # 열 순서 정리
    df_table = df_table[["연월일", "수입지역", "단위가격(톤당달러)", "금액(백만달러)", "중량(톤)"]]

    # 연월일 → YYYY-MM 형식 문자열
    df_table["연월일"] = df_table["연월일"].dt.strftime("%Y-%m")

    # 단위가격: 소수점 첫째 자리까지
    df_table["단위가격(톤당달러)"] = df_table["단위가격(톤당달러)"].round(1)

    # 금액, 중량: 천단위 콤마가 들어간 정수 형식 문자열
    df_table["금액(백만달러)"] = df_table["금액(백만달러)"].round(0)
    df_table["중량(톤)"] = df_table["중량(톤)"].round(0)

    df_table["금액(백만달러)"] = df_table["금액(백만달러)"].apply(
        lambda v: "" if pd.isna(v) else f"{int(v):,}"
    )
    df_table["중량(톤)"] = df_table["중량(톤)"].apply(
        lambda v: "" if pd.isna(v) else f"{int(v):,}"
    )

    st.dataframe(
        df_table,
        use_container_width=True,
        height=450,
    )
